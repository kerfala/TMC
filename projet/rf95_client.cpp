...
int main (int argc, const char* argv[] )
{
  ...
  if (!rf95.init()) {
	...
	//uint8_t data[] = " Bonjour Client Lora ! ";
	//*************************//
	char a[50];
	strcpy(a,argv[1]);   // passage en argument du programme
	uint8_t data[50] ;
	char fin = ' ';
	for (int i=0 ; i <  strlen(argv[1]) ; i++)  // copie des valeurs
		data[i] = a[i];
	for (int i = strlen(argv[1]); i < 50; i++)
		data[i] = (uint8_t)fin;
	//**************************//
